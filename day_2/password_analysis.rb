...
def valid_passwords(string)
  # input string of password requirements and passwords
  # split the array in array of password + req
  passwords_array = string.gsub(':', '').split("\n")
  # split each line in array of 3 elements
  passwords_arrays = passwords_array.map { |string| string.split(' ') }
  # transform first el of each array in array of integer
  passwords_arrays.map! { |array| [array[0].split('-').map(&:to_i), array[1], array[2]] }
  # output integer representing number of valid passwords
  passwords_arrays.select { |array| array[2].count(array[1]) >= array[0][0] && array[2].count(array[1]) <= array[0][1]}.size
end

p valid_passwords(test)
